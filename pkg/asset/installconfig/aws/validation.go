...
func ValidateForProvisioning(client API, ic *types.InstallConfig, metadata *Metadata) error {
	...
	if ic.AWS.HostedZone != "" {
		...
		baseDomainOutput, err := client.GetBaseDomain(context.TODO(), zoneName)
		if err != nil {
			return field.ErrorList{
	...
	if errors = client.ValidateZoneRecords(context.TODO(), zone, zoneName, zonePath, ic); len(errors) > 0 {
		allErrs = append(allErrs, errors...)
	}
