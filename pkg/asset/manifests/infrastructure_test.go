...
func TestGenerateInfrastructure(t *testing.T) {
	cases := []struct {
		...
		installConfig: icBuild.build(
			icBuild.forGCP(),
			icBuild.withGCPServiceEndpoint(gcptypes.ComputeServiceName, "https://googleapis.com/compute/v1/"),
		),
		expectedInfrastructure: infraBuild.build(
...
func (b icBuildNamespace) withGCPServiceEndpoint(name, url string) icOption {
	return func(ic *types.InstallConfig) {
		b.forGCP()(ic)
		ic.Platform.GCP.ServiceEndpoints = append(ic.Platform.GCP.ServiceEndpoints, gcptypes.ServiceEndpoint{Name: name, URL: url})
	}
}
